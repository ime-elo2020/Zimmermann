...
const uint32_t ulValueToSend = 16011; //Alterei a mensagem para meu número de matricula
	...
	for( ;; )
	{
		...
		//Os LEDs irão acender quando o sinal estiver sendo enviado
			  STM_EVAL_LEDOn(LED3);
			  STM_EVAL_LEDOn(LED4);
		//Quando o sinal é enviado, os LEDs 5 e 6 serão desligados, representando que estão aguardando a mensagem
...
uint32_t ulReceivedValue;
	...
	for( ;; )
	{
		...
		// apagar os LEDs 3 e 4 significa que a mensagem foi recebida
		...
		{
			//Os LEDs 5 e 6 confirmam que a mensagem recebida está de acordo com o esperado
			  STM_EVAL_LEDOn(LED5);
			  STM_EVAL_LEDOn(LED6);
		...
		else //se o valor recebido não estiver certo, os LEDs 5 e 6 ficam desligados
		{
			STM_EVAL_LEDOff(LED5);
			STM_EVAL_LEDOff(LED6);
		}
